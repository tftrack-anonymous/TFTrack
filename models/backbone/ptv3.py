...
class PointTransformerV3(PointModule):
    ...
    def forward(self, data_dict):
        ...
        """
        for key in ["coord", "feat", "batch"]:
            if isinstance(data_dict[key], list):
                data_dict[key] = torch.cat(data_dict[key], dim=0)

        if isinstance(data_dict["grid_size"], list):
            data_dict["grid_size"] = data_dict["grid_size"][0]
        elif torch.is_tensor(data_dict["grid_size"]) and data_dict["grid_size"].ndim > 1:
            data_dict["grid_size"] = data_dict["grid_size"][0]
