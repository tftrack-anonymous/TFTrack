...
class PointHead(nn.Module):
    ...
    def __init__(self, q_distribution, use_rot=False):
        super().__init__()
    ...
    def loss(self, results, data_samples):
        ...
        if isinstance(data_samples, (list, tuple)):
            gt_coors = torch.stack([sample['box_label'] for sample in data_samples])
        else:
        ...
        if self.use_rot:
            ...
            if isinstance(data_samples, (list, tuple)):
                gt_rot = torch.stack([sample['theta'] for sample in data_samples])
            else:
